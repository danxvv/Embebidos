		...
		if(imu == '$'){
			while(imu != '\n'){
    			...
    			for(i=0;i<37;i++){
        			...
        			//printf("%s \n",parametros);
        			if(parametros[0]=='$'&&parametros[1]=='P'&&parametros[2]=='C'&&parametros[3]=='H'&&parametros[4]=='R'&&parametros[5]=='A'){
        			//	printf("\nposicion angular \n");
        				strcpy(roll_c,vec[2]);
        				roll = atof(roll_c);
        			...
        			else if(parametros[0]=='$'&&parametros[1]=='P'&&parametros[2]=='C'&&parametros[3]=='H'&&parametros[4]=='R'&&parametros[5]=='R'){
        			//	printf("\nvelocidad angular \n");
        		...
        		//	printf("%f \n",yaw_rate);
        			...
        			vector_pwm[0]=((1/(4*b))*pid_alt)-((1/(4*b*l))*pid_roll)-((1/(4*b*l))*pid_pitch)-((1/(4*k))*pid_yaw);
        			vector_pwm[1]=((1/(4*b))*pid_alt)+((1/(4*b*l))*pid_roll)+((1/(4*b*l))*pid_pitch)-((1/(4*k))*pid_yaw);
        			vector_pwm[2]=((1/(4*b))*pid_alt)+((1/(4*b*l))*pid_roll)-((1/(4*b*l))*pid_pitch)+((1/(4*k))*pid_yaw);
        			vector_pwm[3]=((1/(4*b))*pid_alt)-((1/(4*b*l))*pid_roll)+((1/(4*b*l))*pid_pitch)+((1/(4*k))*pid_yaw);
        			printf("1:%f\n",vector_pwm[0]);
        			printf("2:%f\n",vector_pwm[1]);
        			printf("3:%f\n",vector_pwm[2]);
        			printf("4:%f\n",vector_pwm[3]);
